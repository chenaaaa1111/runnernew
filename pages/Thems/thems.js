            ...
            success: function(recc) {
              ...
              if (recc.cancel) {
                ...
                if (res.data.data.status == 1 && runed == 1) {
                  wx.navigateTo({
                    url: '/pagesB/runnew/runnew?id=' + oldName,
                  })
                } else {
                  wx.navigateTo({
                    url: '/pagesB/runnew/runnew?id=' + oldName,
                ...
                if (res.data.data.status == 0) { //没有选择过为爱而跑
                  wx.navigateTo({
                    url: '/pagesB/forwho/forwho?id=' + name
                  })

                } else if (res.data.data.status == 1 && runed == 1) {
                  wx.navigateTo({
                    url: '/pagesB/runnew/runnew?id=' + name,
                  })
                } else {
                  wx.navigateTo({
                    url: '/pagesB/runnew/runnew?id=' + name,
  ...
  onShow: function() {
    ...
    req.requestThenms({}, function(res) {
      ...
      for (var ind in res.data.data) {
        var imgArra = [];
        var startNum = res.data.data[ind].heng - 2 >= 1 ? res.data.data[ind].heng - 2 : 1;
